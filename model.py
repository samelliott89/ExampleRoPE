...
class KVCache:
    ...
    def empty(
        ...
        return KVCache(
            k=torch.empty(
                batch_size, n_kv_heads, 0, head_dim, device=device, dtype=dtype
            ),
            v=torch.empty(
                batch_size, n_kv_heads, 0, head_dim, device=device, dtype=dtype
            ),
...
class Config:
    ...
    resume_from: str = (
        ""  # path to checkpoint to resume from (e.g., "checkpoints/model_epoch005.pt")
    )
...
def train(config: Config):
    ...
    if config.resume_from and os.path.exists(config.resume_from):
        ...
        if is_main:
            print(
                f"Loaded weights from epoch {start_epoch}, loss was {ckpt.get('loss', 'N/A')}"
            )
    ...
    if is_main:
        ...
        print(
            f"Batch size: {config.batch_size} (x{world_size} GPUs = {config.batch_size * world_size} effective)"
        )
    ...
    if config.use_wandb and is_main:
        wandb.init(
            ...
            | {"n_params": n_params, "device": str(device), "world_size": world_size},
        )
